...
class ThreadPool(object):
    ...
    def ventilate(self, items_to_ventilate):
        ...
        # print(f"Arushi: ventilate() completed. Total ventilated items: {self._ventilated_items}, self._ventilator_queues: {self._ventilator_queues}, q1: {self._ventilator_queues[0].queue}, q2: {self._ventilator_queues[1].queue}, q3: {self._ventilator_queues[2].queue}, q4: {self._ventilator_queues[3].queue}")
        # for i, q in enumerate(self.ventilator_queues):
        #     print(f"Arush: Ventilator Queue: {q.queue}")
        ...
        while True:
            ...
            if all_workers_done and self._shared_results_queue.empty():
                if not self._ventilator or self._ventilator.completed():
                    print(f"Arushi: All workers done, raising EmptyResultError")
            ...
            try:
                ...
                if isinstance(result, VentilatedItemProcessedMessage):
                    ...
                    print(f"Arushi: Returning result from get_results()")
                    return result
            except queue.Empty:
                continue
    ...
    def _round_robin_consumer(self):
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if use_non_blocking:
                    ...
                    item = self._results_queues[current_worker].get(block=True, timeout=300.0)
        ...
        print(f"Arushi: _round_robin_consumer() EXITING - stop_event is set")
        print(f"Arushi: Round-robin consumer exiting after {iteration_count} iterations")
